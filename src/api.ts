...
import type { ArtifactInfo, FileItem, JobInfo, ProjectId } from "./types.js";

const API_BASE = "https://paratranz.cn/api";

function createJsonFormData(buffer: Buffer, fileName: string): FormData {
...
export class ParaTranzApi {
  ...
  async getArtifact(projectId: ProjectId): Promise<ArtifactInfo> {
    ...
    if (!res.ok) throw new Error(`getArtifact ${projectId} failed ${res.status}`);
  ...
  async triggerExport(projectId: ProjectId): Promise<JobInfo> {
    ...
    if (!res.ok) throw new Error(`triggerExport ${projectId} failed ${res.status}`);
  ...
  async downloadArtifact(projectId: ProjectId): Promise<NodeJS.ReadableStream> {
    ...
    if (!res.ok)
      throw new Error(`downloadArtifact ${projectId} failed ${res.status} ${res.statusText}`);
    ...
    if (!res.body) throw new Error(`downloadArtifact ${projectId} returned an empty body`);
    // DOM and Node typings model the same runtime Web stream with incompatible generics.
  ...
  async createFile(
    ...
    const fd = createJsonFormData(buffer, fileName);
    // If need upload to a path, ParaTranz supports "path" field
    const pathOnly = filePath.endsWith("/") ? filePath : filePath ? filePath + "/" : "";
  ...
  async updateFileTranslation(
    ...
    const res = await fetch(`${API_BASE}/projects/${projectId}/files/${fileId}/translation`, {
      method: "POST",
      headers: this.headers(),
      body: fd,
    });
